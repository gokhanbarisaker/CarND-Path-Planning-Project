#include "lcl_trajectory_generator.hpp"
#include <iostream>
#include "../helpers.h"
...
LCLTrajectoryGenerator::LCLTrajectoryGenerator()
{
}

const std::pair<std::vector<double>, std::vector<double>> LCLTrajectoryGenerator::generate(
    ...
    const std::vector<double> &map_waypoints_dy)
{
    ...
    const double s_range = std::max(main_car.speed, 30.0);
...
double LCLTrajectoryGenerator::adjust_speed(const double &speed,
...
double LCLTrajectoryGenerator::adjust_d(const double &d)
{
    // | L1  | L2  | L3  |     | L1  | L2  | L3  |
    // |...x.|.....|.....| --> |..x..|.....|.....|
